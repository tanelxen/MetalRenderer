...
void StudioModel::SetupBodyPart ( int bodypart )
{
    int index;
...
void StudioModel::DrawPoints ( )
{
    ...
    {
        int bone = pvertbone[i];
        VectorTransform (pstudioverts[i], g_bonetransform[bone], g_pxformverts[i]);
    }
...
//    glCullFace(GL_FRONT);
    ...
    for (j = 0; j < m_pmodel->nummesh; j++)
    {
        ...
        mstudiotexture_t *texture = &(ptexture[pskinref[pmesh->skinref]]);
