...
func ImGui_ImplOSX_NewFrame(_ view: NSView)
{
    ...
    io.pointee.DisplaySize = ImVec2(x: Float(view.bounds.size.width), y: Float(view.bounds.size.height))
    ...
    io.pointee.DeltaTime = Float(current_time - g_Time)
    g_Time = current_time
...
func ImGui_ImplOSX_UpdateMouseCursorAndButtons() {
    // Update buttons
    let io = ImGuiGetIO()!
